...
2. 在需要引入配置文件也就是@Value的地方 加上  @RefreshScope

3. bootstrap.yml 加上暴露端口的设置
 management:
  endpoints:
    web:
      exposure:
...
spring:
 rabbitmq:
    ...
    username: lht
    password: 123456
刷新则需要POST方式访问 http://localhost:8771/actuator/bus-refresh
然后就有了


4. 因为与eureka进行集成了，所以当某一个client 访问http://.../actuator/bus-refresh后 
所以的相关client（访问同一个config-server）都会进行更新


！！postman 访问http://localhost:8771/actuator/bus-refresh 时的注意事项

1.headers 里不要有参数

2. Post方式

3. Body 选择 form-data 或者raw



还有就是 config-server 除了@RefreshScope 不用加，其他的配置都要加上 如下 3点
